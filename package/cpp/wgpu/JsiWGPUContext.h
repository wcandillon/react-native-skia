...
public:
  JsiWGPUContext(std::shared_ptr<RNSkPlatformContext> context,
                 std::shared_ptr<WGPUSurfaceDescriptor> m, int width, int height)
      : JsiSkWrappingSharedPtrHostObject<WGPUSurfaceDescriptor>(
            context, std::move(m)), _width(width), _height(height) {}

  JSI_HOST_FUNCTION(configure) {
    auto device = JsiDevice::fromValue(runtime, arguments[0].asObject(runtime).getPropertyAsObject(runtime, "device"));
    auto instance = device->getAdapter().getInstance();
    wgpu::Surface surface = wgpuInstanceCreateSurface(instance, getObject().get());
    ...
    _swapChain = std::make_shared<wgpu::SwapChain>(device->createSwapChain(surface, swapChainDesc));
  ...
  JSI_HOST_FUNCTION(getCurrentTexture) {
    ...
    return jsi::Object::createFromHostObject(runtime, std::make_shared<JsiTexture>(getContext(), texture));
  }

  EXPORT_JSI_API_TYPENAME(JsiWGPUContext, WGPUContext)

  JSI_EXPORT_FUNCTIONS(
    JSI_EXPORT_FUNC(JsiWGPUContext, configure),
